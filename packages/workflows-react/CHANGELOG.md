...
- 6c4133f: Expose persisted workflow schedule execution state on schedule API rows and surface it in the schedules UI when run history is not wired.
...
- eec6cb1: Add an aggregated Schedules view to the workflows UI. The Cloudflare orchestrator now serves `GET /api/schedules/:env`, projecting each workflow's registered schedule blocks with a computed `nextRunAt`, an `enabled` flag, and an optional `schedulesEnabledByEnv` master switch. `@voyant-travel/workflows-react` exports a matching `createWorkflowSchedulesApiClient`, and `@voyant-travel/workflows-ui` ships a `WorkflowSchedulesPage` that lists workflow id, schedule expression, next run, last run (when a runs API is passed), enabled status, and an optional "Trigger now" action.
